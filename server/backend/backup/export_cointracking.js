...
const doJsonExport = require('./import').export
 ...
 * exportdata can be imported in cointracking.info
 *
 ...
 */
let doCointrackingExport = (userId, exportFn = doJsonExport) => {
  return exportFn(userId)
  .then(data => {
    const opts = {fields};

    let csvData = []
    for (let tx of Object.values(data.transactions)) {
      if(handleMap[tx.type]) {
        csvData.push(...handleMap[tx.type](tx))
      }else{
        log.error(`Transaction not supported yet: ${tx.type}`)
      }
    }

    return json2csv.parse(csvData, opts);
  })
}

module.exports = {
  exportCointracking: doCointrackingExport,
};
